...
ap_added_date = as.character.Date(amazon_prime$date_added)
ap_added_date = mdy(ap_added_date)
ap_added_year = year(ap_added_date)
...
amazon_prime <-  amazon_prime %>% 
  mutate(ap_year_diff = ap_added_year-release_year) 

amazon_prime %>% count(ap_year_diff, sort = F)

datatable(
  amazon_prime %>% select(-cast, -description) %>%
    filter(ap_year_diff < 0) %>%
    arrange(ap_year_diff),
  caption = NULL,
  options = list(dom = 't')
)


amazon_prime %>% select(ap_year_diff) %>%
  filter(!is.na(ap_year_diff)) %>%
  plot_ly(x = ~ ap_year_diff,
          type = "histogram",
...
datatable(amazon_prime %>% select(title, type, release_year, date_added, ap_year_diff) %>%
            filter(ap_year_diff > 40) %>% 
            arrange(desc(ap_year_diff)),
          caption = NULL,
          options = list(dom = 't')
)
...
# Rating by Type ----
amazon_prime %>% select(rating, type) %>%
  filter(!is.na(rating)) %>%
